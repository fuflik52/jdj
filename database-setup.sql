...
create table if not exists public.purchased_cards (
    id uuid default uuid_generate_v4() primary key,
    user_id uuid references public.users(id) not null,
    card_id integer references public.cards(id) not null,
...
create policy "Публичный доступ к users" on public.users for select using (true);
create policy "Пользователи могут создавать записи" on public.users for insert with check (true);

create policy "Доступ к своему прогрессу" on public.game_progress for select using (auth.uid() = user_id);
create policy "Сохранение своего прогресса" on public.game_progress for insert with check (auth.uid() = user_id);

create policy "Публичный доступ к cards" on public.cards for select using (true);

create policy "Доступ к своим купленным картам" on public.purchased_cards for select using (auth.uid() = user_id);
create policy "Покупка новых карт" on public.purchased_cards for insert with check (auth.uid() = user_id);
